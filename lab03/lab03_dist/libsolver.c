...
// uintptr_t cal_got_addr(int index) {
//     unsigned long relative_main_addr = 0x1b7a9; 
//     unsigned long actual_got_address = main_address - relative_main_addr + got_offsets[index];
//     // printf("Calculating GOT address: %p\n", (void *)actual_got_address);
//     return actual_got_address;
// }
...
void dfs(maze_t *mz, int *path, int moves){
...
maze_init() {
    ...
    printf("\nUP112_GOT_MAZE_CHALLENGE\n");
    void *lib_handle = dlopen("libmaze.so", RTLD_LAZY);
    if (!lib_handle) {
        fprintf(stderr, "Failed to load the library: %s\n", dlerror());
        return 0;
    }

    int (*mz_init)() = dlsym(lib_handle, "maze_init");
    void *(*mz_get_ptr)() = dlsym(lib_handle, "maze_get_ptr");
	maze_t *(*mz_load)() = dlsym(lib_handle, "maze_load");	

	void (*mv_up)() = dlsym(lib_handle, "move_up");
	void (*mv_down)() = dlsym(lib_handle, "move_down");
	void (*mv_left)() = dlsym(lib_handle, "move_left");
	void (*mv_right)() = dlsym(lib_handle, "move_right");
    ...
    maze_t *mz = mz_load(maze_file);


    int path[1200] = {};
    int moves = 0; 
    dfs(mz, path, moves);
    // void (*move_funcs[1200])(void);  

    unsigned long main_offset = 0x1b7a9;

    for (int i=0; i<total_moves; ++i){
		unsigned long *got_entry = (unsigned long *)((unsigned long)main_ptr - main_offset + got_offsets[i]);
		void *page_start = (void *)((unsigned long)got_entry & ~(getpagesize() - 1));

		void (*move)(maze_t *);

		if (mprotect(page_start, sizeof(unsigned long), PROT_READ | PROT_WRITE) == -1){
			perror("mprotect");
			exit(-1);
		}
		...
		*((unsigned long *)got_entry) = (unsigned long)(move);
